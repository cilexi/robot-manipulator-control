...
void torqueControl(xMateRobot &robot) {
  ...
  // Utils::postureToTransArray(robot.posture(rokae::CoordinateType::flangeInBase, ec), init_position);
    ...
    ofs5.open("pose.txt",std::ios::out | std::ios::trunc);
    ...
    //定义中间向量变量
    //Eigen::Matrix<double, 6, 1> Kp = {200,200,45,100,50,0};
    //Eigen::Matrix<double, 6, 1> Kd = {50,20,10,10,10,0};
    Eigen::Matrix<double, 6, 1> Kp = {200,100,25,50,25,0};
    Eigen::Matrix<double, 6, 1> Kd = {50,20,10,10,10,0};
    Eigen::Matrix<double, 6, 1> k = {0.35,0.6,0.6,0.025,0.125,0};
    ...
    Eigen::Vector3d end_position = start_position + Eigen::Vector3d(0.1, 0.1, 0);
    double trajectory_duration = 2.0;
    
    TrajectoryPoint target = quinticInterpolation(start_position, end_position, time, trajectory_duration);
    
    // 构建完整位姿 - 保持初始姿态
    static Eigen::Quaterniond initial_orientation(initial_transform.linear());
    Eigen::Affine3d target_pose = Eigen::Affine3d::Identity();
    target_pose.translation() = target.position;
    ...
    Eigen::Vector3d position = pose_matrix.block<3,1>(0,3);
        ...
        tau_d <<0*gravity-((Kp.array()*e1.array()+Kd.array()*e2.array()).matrix());
    ...
    //ofs5 << time << " " << pos_e[0] << " " <<pos_e[1] << " " <<pos_e[2] << " " <<pos_e[3] << " " <<pos_e[4] << " " << pos_e[5] << " "  << std::endl;
    if(time > 10){
      ofs1.close();
      ofs2.close();
      ofs3.close();
