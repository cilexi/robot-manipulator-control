#!/usr/bin/env python3
"""Tune, run, analyze, and interactively plot the arm-6 backstepping test."""
...
DEFAULT_PARAMETERS = {
    "k1_6": 62.0,
    "k2_6": 50.0,
    "reference_amplitude6": 0.02,
    "reference_omega6": 0.523598775598,
    "reference_cycles": 2.0,
    "alpha_limit6": 3.30,
    "alpha_dot_limit6": 3.0,
    "virtual_accel_limit6": 28.0,
    "nominal_inertia6": 0.135,
    "iota": 1.0,
    "theta_initial": 0.05,
    "gamma_theta": 1.5,
    "sigma_theta": 1.8,
...
def absolute_error_integral(rows):
    ...
    if len(rows) < 2:
        return 0.0
    return sum(
        0.5 * (abs(previous["error6"]) + abs(current["error6"]))
...
def read_rows(path):
    ...
    if not rows:
        raise ValueError("arm6_backstepping 日志没有数据行")
    return rows
...
def replace_joint6(text, key, value):
    pattern = re.compile(rf"^(\s*{re.escape(key)}\s*=\s*)([^#\r\n]+)$", re.MULTILINE)
    match = pattern.search(text)
    if not match:
        raise ValueError(f"找不到六轴配置项 {key}")
    values = match.group(2).split()
    if len(values) != 6:
        raise ValueError(f"配置项 {key} 必须包含 6 个数")
    values[5] = f"{value:.10g}"
...
def effective_parameters(args):
    return {
        "k1_6": args.k1,
        "k2_6": args.k2,
        "reference_amplitude6": args.amplitude,
        "reference_omega6": args.omega,
        "reference_cycles": args.cycles,
        "alpha_limit6": args.alpha_limit,
        "alpha_dot_limit6": args.alpha_dot_limit,
        "virtual_accel_limit6": args.virtual_accel_limit,
        "nominal_inertia6": args.inertia6,
        "iota": args.iota,
        "theta_initial": args.theta_initial,
        "gamma_theta": args.gamma_theta,
        "sigma_theta": args.sigma_theta,
        "tau_limit6": args.tau_limit6,
...
def print_parameters(params):
    descriptions = {
        "k1_6": "第一层反馈增益",
        "k2_6": "第二层反馈增益",
        "reference_amplitude6": "第六关节正弦幅值 rad",
        "reference_omega6": "第六关节角频率 rad/s",
        "reference_cycles": "完整正弦周期数",
        "alpha_limit6": "虚拟速度上限 rad/s",
        "alpha_dot_limit6": "虚拟速度导数上限 rad/s^2",
        "virtual_accel_limit6": "虚拟加速度上限 rad/s^2",
        "nominal_inertia6": "第六关节标称惯量",
        "iota": "鲁棒项设计参数",
        "theta_initial": "theta 初值",
        "gamma_theta": "theta 自适应增益",
        "sigma_theta": "theta 泄漏系数",
        "tau_limit6": "第六关节力矩上限 Nm",
        "enable_output_barrier": "输出障碍开关",
        "enable_theta_adaptation": "theta 自适应开关",
    }
    print("\n本次 arm6_backstepping 参数")
    print("-" * 78)
    for key, value in params.items():
        display = str(value) if isinstance(value, bool) else f"{value:.9g}"
        print(f"{key:<31}{display:>14}  {descriptions[key]}")
    print("-" * 78)
    print("长期默认值可在 arm6_backstepping_tune_run.py 顶部 DEFAULT_PARAMETERS 中修改。\n")
...
def make_run_config(base_text, params):
    ...
    for key, value in params.items():
        if isinstance(value, bool):
            ...
            text = replace_joint6(text, "tau_limit", value)
        else:
...
def analyze(rows):
    ...
    positive = [row for row in active if row["dqd6"] > 1.0e-5]
    negative = [row for row in active if row["dqd6"] < -1.0e-5]

    def directional(source):
        return {
            "samples": len(source),
            "error_rms": rms([row["error6"] for row in source]),
            "error_mean": mean([row["error6"] for row in source]),
            "tau_mean": mean([row["tau_cmd6"] for row in source]),
        }

    held_drift = {}
    for joint in (1, 2, 3, 4, 5):
    ...
    metrics = {
        ...
        "error_rms": rms([row["error6"] for row in active]),
        "error_peak": max(abs(row["error6"]) for row in rows),
        "z1_rms": rms([row["z1"] for row in active]),
        "z2_rms": rms([row["z2"] for row in active]),
        "alpha_peak": max(abs(row["alpha6"]) for row in rows),
        "alpha_dot_peak": max(abs(row["alpha_dot6"]) for row in rows),
        "virtual_accel_peak": max(abs(row["virtual_accel6"]) for row in rows),
        "tau_raw_peak": max(abs(row["tau6_raw"]) for row in rows),
        "tau_cmd_peak": max(abs(row["tau_cmd6"]) for row in rows),
        "barrier_ratio_peak": max(abs(row["barrier_ratio6"]) for row in rows),
        ...
        "torque_amplitude_saturation_fraction": fraction(
            row["amplitude_saturated6"] != 0.0 for row in rows
        ),
        "torque_rate_saturation_fraction": fraction(
            row["rate_saturated6"] != 0.0 for row in rows
        ),
...
def plot_interactive(rows, metrics, params, output_path=None):
    ...
    fig.suptitle(
        "Joint 6 No-RL Backstepping\n"
        f"K1={params['k1_6']:.4g}, K2={params['k2_6']:.4g}, "
        f"A={params['reference_amplitude6']:.4g} rad, "
        f"omega={params['reference_omega6']:.4g} rad/s",
        fontsize=14,
    )

    axes[0, 0].plot(time, [row["q6"] for row in rows], label="q6")
    axes[0, 0].plot(time, [row["qd6"] for row in rows], "--", label="qd6")
    axes[0, 0].set_title("Trajectory and reference")
    axes[0, 0].set_ylabel("position (rad)")
    axes[0, 0].legend()

    axes[0, 1].plot(time, [row["error6"] for row in rows], label="q6-qd6")
    ...
    axes[1, 0].plot(time, [row["dq6"] for row in rows], label="dq6")
    axes[1, 0].plot(time, [row["alpha6"] for row in rows], "--", label="alpha6")
    axes[1, 0].set_title("Second-layer state and virtual velocity")
    axes[1, 0].set_ylabel("rad/s")
    axes[1, 0].legend()

    axes[1, 1].plot(time, [row["alpha_dot6"] for row in rows], label="alpha_dot")
    axes[1, 1].plot(time, [row["term_z2_6"] for row in rows], label="-K2*z2")
    axes[1, 1].plot(time, [row["term_mu_6"] for row in rows], label="-mu_s")
    axes[1, 1].plot(time, [row["term_nu_6"] for row in rows], label="-nu_r")
    axes[1, 1].plot(
        time, [row["virtual_accel6"] for row in rows],
    ...
    axes[2, 0].plot(time, [row["tau6_raw"] for row in rows], label="backstepping tau6")
    axes[2, 0].plot(time, [row["tau_cmd6"] for row in rows], label="command tau6")
    axes[2, 0].axhline(
        params["tau_limit6"], color="red", linestyle="--", linewidth=0.9
    )
    axes[2, 0].axhline(
        -params["tau_limit6"], color="red", linestyle="--", linewidth=0.9
    )
    axes[2, 0].set_title("Joint-6 torque")
    ...
    barrier_axis.plot(
        time, [row["barrier_ratio6"] for row in rows],
...
def build_parser(script_dir, repo):
    parser = argparse.ArgumentParser(
        description="运行、调参并显示第六关节 No-RL 反步实验"
    )
    parser.add_argument(
        "--config", type=Path, default=script_dir / "arm6_backstepping.cfg"
    )
    parser.add_argument(
    ...
    parser.add_argument("--k1", type=float, default=DEFAULT_PARAMETERS["k1_6"])
    parser.add_argument("--k2", type=float, default=DEFAULT_PARAMETERS["k2_6"])
    parser.add_argument(
        "--amplitude", type=float, default=DEFAULT_PARAMETERS["reference_amplitude6"]
    )
    parser.add_argument(
        "--omega", type=float, default=DEFAULT_PARAMETERS["reference_omega6"]
    )
    parser.add_argument(
        "--cycles", type=float, default=DEFAULT_PARAMETERS["reference_cycles"]
    )
    parser.add_argument(
        "--alpha-limit", type=float, default=DEFAULT_PARAMETERS["alpha_limit6"]
    )
    parser.add_argument(
        "--alpha-dot-limit", type=float,
        default=DEFAULT_PARAMETERS["alpha_dot_limit6"]
    )
    parser.add_argument(
        "--virtual-accel-limit", type=float,
        default=DEFAULT_PARAMETERS["virtual_accel_limit6"]
    )
    parser.add_argument(
        "--nominal-inertia6", "--inertia6", dest="inertia6", type=float,
        default=DEFAULT_PARAMETERS["nominal_inertia6"],
        help="第六关节标称惯量，即论文控制律中的 B 的逆"
    )
    ...
    parser.add_argument(
        "--tau-limit6", type=float, default=DEFAULT_PARAMETERS["tau_limit6"]
    )
    parser.add_argument(
...
def main():
    ...
    if args.log:
        ...
        if not log_path.is_file():
            ...
            return 2
        sibling_cfg = next(log_path.parent.glob("arm6-*-config.cfg"), None)
        if sibling_cfg is not None and sibling_cfg.is_file():
        ...
        plot_interactive(
            rows, metrics, params,
            output_path=log_path.parent / "arm6-analysis-result.png",
        )
        return 0
    ...
    m_token = f"{params['nominal_inertia6']:.10g}".replace(".", "p")
    run_name = (
        f"arm6-{datetime.now():%Y%m%d-%H%M%S}"
        f"-k1-{params['k1_6']:.10g}-k2-{params['k2_6']:.10g}-M-{m_token}"
    )
    run_dir = script_dir.parent / "records" / run_name
    run_dir.mkdir(parents=True)
    file_stem = (
        f"arm6-k1-{params['k1_6']:.10g}"
        f"-k2-{params['k2_6']:.10g}-M-{m_token}"
    )
    run_cfg = run_dir / f"{file_stem}-config.cfg"
    ...
    (run_dir / f"{file_stem}-check-output.txt").write_text(
    ...
    (run_dir / f"{file_stem}-console.txt").write_text(console, encoding="utf-8")
    raw_log_path = run_dir / "arm6_backstepping_log.csv"
    log_path = run_dir / f"{file_stem}-log.csv"
    if raw_log_path.is_file():
        raw_log_path.rename(log_path)
    if not log_path.is_file():
        print("未生成 arm6_backstepping_log.csv。", file=sys.stderr)
        return 4
    rows = read_rows(log_path)
    metrics = analyze(rows)
    metrics["return_code"] = return_code
    (run_dir / f"{file_stem}-metrics.json").write_text(
    ...
    plot_interactive(
        rows, metrics, params,
        output_path=run_dir / f"{file_stem}-result.png",
    )
