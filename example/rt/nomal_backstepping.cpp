/**
 * @file nomal_backstepping.cpp
 * @brief Compact no-learning backstepping experiment for physical joint 5.
 *
 ...
 *   3) z2=dq5-alpha and robust adaptive term nu_r;
 *   4) scalar nominal inertia mapping tau5=J5*v.
 *
 * Actor/critic networks, RBF features, Bellman residuals, pseudo-Huber loss,
 * weights, and every learning update are intentionally absent.  Joints 1-4 and 6
 * use a small engineering PD hold around q_init so the joint-5 scalar reduction
...
constexpr std::size_t kJoint5 = 4;
...
struct Config {
  ...
  // qd5=q_init5+A*sin(omega*t), followed by a center hold.
  double reference_amplitude5 = 0.02;
  double reference_omega5 = 0.523598775598;
  double reference_cycles = 2.0;
  double final_hold_time = 3.0;

  // Relative displacement constraint around q_init5.
  bool enable_output_barrier = true;
  double output_bound5 = 0.10;
  ...
  double k1_5 = 6.0;
  double k2_5 = 10.0;
  double alpha_limit5 = 0.30;
  double alpha_dot_limit5 = 3.0;
  double alpha_dot_filter = 0.02;
  double virtual_accel_limit5 = 8.0;
  ...
  // Explicit scalar input map for joint 5.  This replaces the large embedded
  // URDF calculator and must be identified/checked for the actual setup.
  double nominal_inertia5 = 0.02;//论文控制率中的B的倒数，即惯性矩阵

  // Engineering PD hold for joints 1-4 and 6; not part of the joint-5 law.
  double hold_kp = 2.0;
  double hold_kd = 0.10;
  ...
  double joint5_abort_error = 0.08;
  double joint5_displacement_limit = 0.10;
  Vec6 abort_velocity{0.30, 0.30, 0.30, 0.30, 0.30, 0.30};
  Vec6 hold_joint_drift_limit{0.02, 0.02, 0.02, 0.02, 0.0, 0.02};
...
#define SET_VEC(name)                                                          \
  ...
  SET_SCALAR(joint5_abort_error)
  SET_SCALAR(joint5_displacement_limit)
...
double sineDuration(const Config &cfg) {//计算正弦轨迹的持续时间
  return cfg.reference_cycles * 2.0 * kPi / cfg.reference_omega5;
}
...
void validateConfig(const Config &cfg) {//验证配置项的有效性，如果不符合要求则抛出异常
  ...
  requirePositive(cfg.reference_amplitude5, "reference_amplitude5");
  requirePositive(cfg.reference_omega5, "reference_omega5");
  ...
  requirePositive(cfg.output_bound5, "output_bound5");
  ...
  if (cfg.enable_output_barrier &&
      cfg.reference_amplitude5 >=
          cfg.barrier_abort_ratio * cfg.output_bound5) {
    throw std::runtime_error("第五关节参考轨迹可能触及障碍停机边界");
  }
  requirePositive(cfg.k1_5, "k1_5");
  requirePositive(cfg.k2_5, "k2_5");
  requirePositive(cfg.alpha_limit5, "alpha_limit5");
  requirePositive(cfg.alpha_dot_limit5, "alpha_dot_limit5");
  if (!(cfg.alpha_dot_filter > 0.0 && cfg.alpha_dot_filter <= 1.0)) {
    throw std::runtime_error("alpha_dot_filter 必须位于 (0,1]");
  }
  requirePositive(cfg.virtual_accel_limit5, "virtual_accel_limit5");
  ...
  requirePositive(cfg.nominal_inertia5, "nominal_inertia5");
  ...
  requirePositive(cfg.joint5_abort_error, "joint5_abort_error");
  requirePositive(cfg.joint5_displacement_limit,
                  "joint5_displacement_limit");
  if (cfg.reference_amplitude5 >= 0.8 * cfg.joint5_displacement_limit) {
    throw std::runtime_error(
        "reference_amplitude5 必须小于 joint5_displacement_limit 的 80%");
  }
...
std::string defaultConfigPath() {//返回默认配置文件路径
  const std::array<std::string, 5> candidates{
      "nomal_backstepping.cfg", "config/nomal_backstepping.cfg",
...
Desired desiredTrajectory(double time, double q_init5, const Config &cfg) {//计算期望轨迹，根据时间和配置参数生成正弦轨迹
  Desired desired;
  desired.q = q_init5;
  if (time < sineDuration(cfg)) {
    const double angle = cfg.reference_omega5 * time;//计算当前时间对应的角度
    desired.q += cfg.reference_amplitude5 * std::sin(angle);//计算期望位置
    desired.dq =
        cfg.reference_amplitude5 * cfg.reference_omega5 * std::cos(angle);
    desired.ddq = -cfg.reference_amplitude5 * cfg.reference_omega5 *
                  cfg.reference_omega5 * std::sin(angle);
...
class NoRlBacksteppingController {
 public:
  NoRlBacksteppingController(const Config &cfg, double q_init5)
      : cfg_(cfg), q_init5_(q_init5), theta_hat_(cfg.theta_initial) {}

  BacksteppingOutput compute(double time, double q5, double dq5) {
    BacksteppingOutput output;
    output.desired = desiredTrajectory(time, q_init5_, cfg_);//计算期望轨迹
    output.error = q5 - output.desired.q;
    ...
    if (cfg_.enable_output_barrier) {//如果启用输出障碍变换，则计算变换后的值
      ...
      const double lambda =//计算lambda值，固定边界情况下正确
          ...
          cfg_.barrier_bf;
      const BarrierScalar actual =//相对实验起点的障碍变换
          barrierTransform(q5 - q_init5_, dq5, lambda, beta, beta_dot,
                           cfg_.barrier_b0);
      const BarrierScalar desired = barrierTransform(
          output.desired.q - q_init5_, output.desired.dq, lambda,
    ...
    const double alpha_raw =
        (-cfg_.k1_5 * output.z1 - output.hbeta_x + sd_dot) /
        (output.hx + kEps);//论文虚拟控制律：Hx^{-1}(-K1*z1-h_beta_x+s_dot_d)
    output.alpha = clamp(alpha_raw, -cfg_.alpha_limit5, cfg_.alpha_limit5);
    ...
    output.alpha_dot = clamp(alpha_dot_filtered_, -cfg_.alpha_dot_limit5,
                             cfg_.alpha_dot_limit5);//限制后的滤波alpha导数
    output.alpha_dot_saturated =//
        std::abs(output.alpha_dot - alpha_dot_filtered_) > kEps;

    output.z2 = dq5 - output.alpha;
    output.phi = 1.0 + std::abs(q5) + std::abs(dq5) + 0.25 * dq5 * dq5;
    ...
    output.term_z2 = -cfg_.k2_5 * output.z2;//论文中的term_z2=-K2*z2
    ...
    output.virtual_accel =//做个限制
        clamp(output.virtual_accel_unsaturated, -cfg_.virtual_accel_limit5,
              cfg_.virtual_accel_limit5);
    output.virtual_accel_saturated =//留个判断是否到达限制
        std::abs(output.virtual_accel - output.virtual_accel_unsaturated) >
        kEps;
    output.torque5_raw = cfg_.nominal_inertia5 * output.virtual_accel;
    ...
    output.finite =
        ...
        isFinite(output.theta_hat) && isFinite(output.torque5_raw);
    return output;
  }
...
LimitedTorque buildAndLimitTorque(const BacksteppingOutput &control,
  ...
  output.raw[kJoint5] = control.torque5_raw;
...
SafetyStatus checkSafety(const Vec6 &q, const Vec6 &dq, const Vec6 &q_init,
  ...
  if (std::abs(control.error) > cfg.joint5_abort_error) {
    return {true, "关节 5 跟踪误差超限"};
  }
  if (std::abs(q[kJoint5] - q_init[kJoint5]) >
      cfg.joint5_displacement_limit) {
    return {true, "关节 5 偏离实验起点超过允许范围"};
  }
  if (cfg.enable_output_barrier &&
      std::abs(control.barrier_ratio) > cfg.barrier_abort_ratio) {
    return {true, "关节 5 接近输出约束边界"};
  }
  return {};
}
...
void writeLogHeader(std::ofstream &log) {//写入日志文件的表头，包括时间、关节状态、控制器输出和力矩命令
  ...
  log << ",qd5,dqd5,ddqd5,error5,z1,z2,hx,hbeta_x5,barrier_ratio5,alpha5"
         ",alpha_dot5,mu_s5,phi5,nu_r5,term_z2_5,term_mu_5,term_nu_5"
         ",virtual_accel_unsat5,virtual_accel5,theta_hat,tau5_raw";
  ...
  log << ",alpha_saturated,alpha_dot_saturated,virtual_accel_saturated"
         ",amplitude_saturated5,rate_saturated5,abort\n";
}
...
void writeLogRow(std::ofstream &log, double time, const Vec6 &q,
  ...
  log << ',' << control.desired.q << ',' << control.desired.dq << ','
      ...
      << ',' << control.theta_hat << ',' << control.torque5_raw;
  ...
  log << ',' << (control.alpha_saturated ? 1 : 0) << ','
      ...
      << torque.amplitude_saturated[kJoint5] << ','
      << torque.rate_saturated[kJoint5] << ',' << (abort ? 1 : 0) << '\n';
}
...
void printConfigSummary(const Config &cfg, const std::string &path) {//打印配置摘要信息，包括关节数、是否启用输出障碍、控制参数和参考轨迹参数
  std::cout << "nomal_backstepping 配置检查通过: " << path << '\n'
            << "  joint=5, RL/NN=absent, barrier="
            << (cfg.enable_output_barrier ? "true" : "false") << '\n'
            << "  K1=" << cfg.k1_5 << ", K2=" << cfg.k2_5 << '\n'
            << "  reference A=" << cfg.reference_amplitude5
            << " rad, omega=" << cfg.reference_omega5 << " rad/s\n"
            << "  nominal_inertia5=" << cfg.nominal_inertia5
            << ", v_limit=" << cfg.virtual_accel_limit5
            << ", tau_limit5=" << cfg.tau_limit[kJoint5] << " Nm\n"
...
bool runExperiment(rokae::xMateRobot &robot, const Config &cfg) {//运行实验函数，连接机械臂并执行实时控制
  ...
  if (cfg.perform_ready_move) {
    ...
    std::cout << "跳过 MoveJ，当前姿态作为第五关节正弦中心和其余关节保持位置。"
              << std::endl;
  }
  ec.clear();
  const Vec6 q_init = robot.jointPos(ec);
  throwIfError(ec, "读取实时控制初始位姿");
  NoRlBacksteppingController controller(cfg, q_init[kJoint5]);
  if (!rt_controller->setFilterLimit(true, cfg.sdk_filter_cutoff_hz)) {
    throw std::runtime_error("SDK 力矩滤波参数设置失败");
  }
  std::ofstream log("arm5_backstepping_log.csv", std::ios::trunc);
  if (!log.is_open()) {
    throw std::runtime_error("无法创建 arm5_backstepping_log.csv");
  }
  writeLogHeader(log);
  rt_controller->startMove(RtControllerMode::torque);
  std::cout << "已进入第五关节 No-RL 反步力矩实验。" << std::endl;
  ...
  std::function<Torque(void)> callback = [&]() -> Torque {
    ...
    SafetyStatus safety;
    if (state_read_ok) {
      control = controller.compute(time, q[kJoint5], dq[kJoint5]);
    ...
    if (tick % 500 == 0) {
      std::cout << "t=" << time << " e5=" << control.error
                << " z1=" << control.z1 << " z2=" << control.z2
                << " tau5=" << torque.command[kJoint5]
  ...
  if (experiment_aborted) {
    ...
    std::cout << "实验完成。日志: arm5_backstepping_log.csv" << std::endl;
  }
